...
def click():
	...
	while True:
		...
		if swt_value <= 10 and isSWTclick == False and state == 0:
			isSWTclick = True
...
def click2():
	...
	while True:
		...
		if swt_value <= 10 and isSWTclick == False and position_R == -1:
			isSWTclick = True
			if R_pase == 0:
				R_pase = 1
				window.mFlagV = True
...
class SlideShowPics(QtGui.QMainWindow):
	...
	def eventCos(self, event):
		if event == QtCore.Qt.Key_Escape:
		...
		elif self.mFlagC==True and event == 10:#click 
			...
			if self.stC == 0:
				if event == -1 or event == 0 or event == 2 or event == 4 or event == 6: #change main screen after insert Nothing
					self.changeImgC()
			elif self.stC == 1:				
				if event == -1:
					self.stC = 0
