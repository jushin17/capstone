...
class OBDPort:
     ...
     def get_sensor_value(self,sensor):
         """Internal use only: not a public interface"""
         cmd = sensor.cmd  #cmd 는 0100 같은 hex 코드들??
     ...
     def sensor(self , sensor_index): #sensor_index 는 하나의 sensor
         """Returns 3-tuple of given sensors. 3-tuple consists of
         (Sensor Name (string), Sensor Value (string), Sensor Unit (string) ) """
         #input으로 주어진 sensor에 맞는 결과를 3-tuple 형태로 돌려준
         sensor = obd_sensors.SENSORS[sensor_index] #obd_sensors 모듈에 SENSORS는 센서 리스트로 정확히 가져온다.
         #ex)  Sensor("speed"                 , "           Vehicle Speed", "010D1", speed            ,"MPH"    ),
