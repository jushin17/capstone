...
class OBD_Recorder():
    def __init__(self, path, log_items):
        ...
        filename = path+"keyboard-"+str(localtime[0])+"-"+str(localtime[1])+"-"+str(localtime[2])+"-"+str(localtime[3])+"-"+str(localtime[4])+"-"+str(localtime[5])+".log"
    ...
    def record_data(self):
        ...
        while 1: #무한루프
            ...
            for index in self.sensorlist:
                (name, value, unit) = self.port.sensor(index) #self.port 는 obd_io class
            ...
            for key in results:
                if key == 'speed' or key == 'gear' or key == 'steering':
                    CDataStore.SensorData[key] = results[key]            
